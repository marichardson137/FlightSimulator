...
int main(void)
{
    ...
    Matrix inertia = {
        .m0 = 4853.0f, .m1 = -132.0f, .m2 = 0.0f, .m3 = 0.0f, .m4 = -132.0f, .m5 = 25660.0f, .m6 = 0.0f, .m7 = 0.0f, .m8 = 0.0f, .m9 = 0.0f, .m10 = 21133.0f, .m11 = 0.0f, .m12 = 0.0f, .m13 = 0.0f, .m14 = 0.0f, .m15 = 1.0f
    };
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_F1))
            debug.showCoordinateSystem = !debug.showCoordinateSystem;
        if (IsKeyPressed(KEY_F2))
            debug.showForces = !debug.showForces;
        if (IsKeyPressed(KEY_F3))
            debug.showWings = !debug.showWings;
        if (IsKeyPressed(KEY_F4))
            debug.showVelocity = !debug.showVelocity;
        if (IsKeyPressed(KEY_F5))
            debug.showControlInputs = !debug.showControlInputs;
        ...
        if (!pauseSimulation) {
            // Throttle control
            if (IsKeyDown(KEY_W))
                plane.engine.throttle += 0.5f * deltaTime;
            if (IsKeyDown(KEY_S))
                plane.engine.throttle -= 0.5f * deltaTime;
