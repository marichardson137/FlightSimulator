...
void ApplyForceWing(Wing* wing, RigidBody* rb, float dt)
{
    ...
    if (speed < 0.1f)
        return; // Skip if almost stationary
...
void ApplyForceDrag(RigidBody* rbp)
{
    Vector3 velocity = rbp->velocity;
    float speed = Vector3Length(velocity);

    // Drag = -0.5 * Cd * rho * A * v^2 * direction
    float dragCoefficient = 0.027f; // typical for a streamlined body
    float airDensity = CalculateAirDensity(rbp->position.y);
    float referenceArea = 20.0f; // approximate frontal area in m^2

    Vector3 dragDirection = Vector3Normalize(Vector3Negate(velocity));
    Vector3 dragForce = Vector3Scale(dragDirection, 0.5f * dragCoefficient * airDensity * referenceArea * speed * speed);

    // Apply the drag force
    rbp->force = Vector3Add(rbp->force, dragForce);
}
...
void UpdatePlane(Plane* plane, float dt)
{
    ...
    // Apply the drag force from the body of the plane
    ApplyForceDrag(&plane->rb);
